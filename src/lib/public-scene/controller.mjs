...
export function mountPageScene(root,load=()=>import('./renderer')){
  ...
  let renderer=null,token=0,loading=false,visible=false,closed=false,ended=false,abort=null,playing=false,interactive=false;
  const button=name=>$(`[data-scene-action="${name}"]`);
  function release(){token++;abort?.abort();abort=null;renderer?.dispose();renderer=null;host.replaceChildren();loading=false;playing=false;interactive=false;controls.hidden=true;fallback.hidden=false;launch.hidden=false;launch.disabled=false;launch.textContent='Explore in 3D ↗';badge.textContent='3D EXPLORER';button('play').textContent='Play motion';button('play').setAttribute('aria-pressed','false');if(button('interact')){button('interact').textContent='Enable drag';button('interact').setAttribute('aria-pressed','false');}}
  ...
  async function start(){
    ...
    try{
      ...
      renderer=made;renderer.setVisible(visible);loading=false;launch.hidden=true;launch.disabled=false;fallback.hidden=true;controls.hidden=false;badge.textContent='INTERACTIVE CONCEPT';
  ...
  const observer=typeof IntersectionObserver==='function'?new IntersectionObserver(entries=>{
    visible=entries.some(entry=>entry.isIntersecting);renderer?.setVisible(visible);
