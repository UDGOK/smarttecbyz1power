...
Static by default. Every public page prerenders and is served as a file. The
on-demand surface is `/api/news`, `/api/investor/[action]`, and the four
private investor pages: login, room, immersive pitch and PDF reader.
...
Output is 11 prerendered HTML pages plus hashed assets, and one server bundle
...
- Set Node to 24 in Project Settings → General → Node.js Version
- Static pages are served from the edge; private pages and API endpoints run in the serverless bundle
