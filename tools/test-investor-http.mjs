...
let r=await fetch(origin+'/investors',{redirect:'manual'});assert.equal(r.status,303);checks++;assert.equal(r.headers.get('location'),'/investors/login');assert.match(r.headers.get('cache-control'),/no-store/);assert.match(r.headers.get('cdn-cache-control'),/no-store/);assert.equal(r.headers.get('x-frame-options'),'DENY');checks++;
...
for(const path of ['presentation','bootstrap','model','concept-manufacturing','concept-compute','concept-energy','module-factory','module-rack','module-energy',...pitchMedia.assets.map(asset=>asset.action)]){r=await fetch(origin+'/api/investor/'+path);assert.equal(r.status,401);checks++;}
