...
test('anonymous APIs and survey downloads deny access',async()=>{for(const action of ['bootstrap','model','calculate','export','faq','survey','survey-image']){const r=await handle(req(action),action,{cfg,store:new Store()});assert.equal(r.status,401,action);assert.equal(r.headers.get('cache-control'),privateHeaders['Cache-Control']);}});
...
test('authenticated model GET and HEAD expose only the reviewed snapshot',async()=>{
 ...
 const head=await handle(req('model',undefined,{cookie},'HEAD'),'model',{cfg,store});assert.equal(head.status,200);assert.equal((await head.arrayBuffer()).byteLength,0);assert.match(head.headers.get('content-disposition'),/SmartTec_Model_v6\.1_Verified_Scenarios\.json/);assert.match(head.headers.get('cache-control'),/no-store/);
 const bootstrap=await(await handle(req('bootstrap',undefined,{cookie}),'bootstrap',{cfg,store})).json();assert.deepEqual(bootstrap.model,model);assert.equal(bootstrap.sample,undefined);
});
...
test('FAQ cites the current model and unknown questions stay unknown',()=>{
 ...
 const funding=answer.matches.find(f=>f.id==='F17');assert.ok(funding);assert.ok(funding.answer.includes(usd(base.capital.totalUsd)));assert.ok(funding.sourceIds.includes('model-v6.1'));assert.ok(funding.sources.some(source=>source.title.includes(model.source.sha256)));
