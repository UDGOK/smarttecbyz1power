...
test('downloaded model retains exact reviewed scenarios, source provenance and private headers',async()=>{
 ...
 assert.match(response.headers.get('content-disposition'),/^attachment; filename="SmartTec_Management_Target_2026-09-14\.json"$/);
 ...
 const snapshot=await response.json();assert.deepEqual(snapshot,model);assert.equal(snapshot.version,'2026.09-target');assert.match(snapshot.source.sha256,/^[a-f0-9]{64}$/i);assert.ok(snapshot.reviewedAt);
 assert.deepEqual(snapshot.scenarios.map(s=>s.id),['base','slower-ramp','price-pressure']);
 assert.equal(snapshot.assumptions.primaryReturnMetric,'headlineIrr');
 assert.equal(snapshot.source.baseWorkbook.sha256,'60ef7ae93c731fecee74e31241dd5e2ac3cf669bb74a0668d57f77796aa179aa');
});
