...
import {thermalAsset} from '../src/smarttec-architecture/server/thermal-assets.mjs';

test('each chapter has its own appropriate image and a valid self-contained model',async()=>{
 assert.equal(new Set(Object.values(chapters).map(c=>c.image)).size,5);
 for(const [key,c] of Object.entries(chapters)){
  assert.ok(c.alt.length>40);assert.ok(c.modelNote);assert.ok(imageDisclosure(key));
  const picture=thermalAsset(c.image,'GET')||architectureAsset(c.image,'GET');
  if(key==='cooling'){assert.equal(picture.mime,'image/webp');assert.equal(picture.bytes.toString('ascii',8,12),'WEBP');}
  else{assert.equal(picture.mime,'image/png');assert.equal(picture.bytes.toString('ascii',1,4),'PNG');}
  const {bytes}=thermalAsset(c.model,'GET')||architectureAsset(c.model,'GET');assert.equal(bytes.toString('ascii',0,4),'glTF');assert.equal(bytes.readUInt32LE(8),bytes.length);
  ...
  let triangles=0;model.scene.traverse(o=>{if(o.isMesh){triangles+=(o.geometry.index?.count??o.geometry.attributes.position.count)/3;o.geometry.dispose();o.material.dispose();}});assert.ok(triangles>100);
 }
});
